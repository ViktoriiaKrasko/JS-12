...
function createDictionary(entries) {
  ...
  return dictionary; // Використання методу forEach для перебору масиву пар ключ-значення
...
function setValue(dictionary, key, value) {
  ...
  return dictionary; // Перевірка, чи існує вже ключ у словнику
...
function deleteKey(dictionary, key) {
  if (dictionary.has(key)) {
    ...
    return false; // Перевірка, чи існує ключ у словнику
...
function getKeysStartingWith(dictionary, letter) {
  const newArray = []; // Оголошення порожнього масиву для збереження ключів
  for (const key of dictionary.keys()) {
    if (key.startsWith(letter)) {
      newArray.push(key);
    }
  }
  return newArray; // Перебір ключів словника за допомогою циклу for...of
...
function addKeyValuePairs(dictionary, entries) {
  ...
  return { dictionary, added, rejected }; // Використовуємо метод forEach для перебору масиву пар ключ-значення
...
function checkPresence(dictionary, keys) {
  const result = []; // Створюємо порожній масив для збереження результатів перевірки
  keys.forEach((key) =>
    dictionary.has(key) ? result.push(true) : result.push(false)
  ...
  return result; // Повертаємо масив
}
...
function getFilteredDictionarySize(dictionary, filter) {
  const map = new Map(); // Створюємо новий Map об'єкт для зберігання елементів, що відповідають фільтру
...
function sortByValues(dictionary) {
  const newArray = [...dictionary.entries()]; // Конвертуємо словник в масив пар ключ-значення за допомогою оператора деструктурізації
  newArray.sort((value1, value2) => value2[1] - value1[1]); // Сортуємо масив пар ключ-значення за значеннями в порядку спадання
  const result = new Map(newArray);
  return result; // Конвертуємо відсортований масив пар ключ-значення назад у словник
}
...
function convertDictionaryToSet(dictionary) {
  ...
  return set; // Перебираємо ключі словника
...
function convertSetToDictionary(set) {
  ...
  set.forEach((key) => {
    const symbol = key.charCodeAt(0);
    dictionary.set(key, symbol);
  }); // Перебираємо елементи множини
  return dictionary; // Додаємо елемент в словник з ключем, який дорівнює елементу, та значенням, яке дорівнює коду його першого символу
